      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex-shrink-0">
            <a href="#home" className="text-2xl font-bold text-primary-light dark:text-primary-light">
              PAR
            </a>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <a
                ...
                className={`relative text-gray-700 dark:text-gray-300 hover:text-primary-light dark:hover:text-primary-light transition-all duration-300 font-medium group ${
                  activeSection === link.href.substring(1) ? 'text-primary-light' : ''
                }`}
              >
                {link.name}
            ...
            <button
              onClick={toggleTheme}
              className="p-2 rounded-lg bg-gray-200 dark:bg-dark-hover hover:bg-gray-300 dark:hover:bg-gray-600 transition-all duration-300 transform hover:scale-110 hover:rotate-12 ripple"
          ...
          <div className="md:hidden flex items-center space-x-4">
            <button
              onClick={toggleTheme}
              className="p-2 rounded-lg bg-gray-200 dark:bg-dark-hover transition-all duration-300 transform hover:scale-110 ripple"
              aria-label="Toggle theme"
            >
            ...
            {navLinks.map((link, index) => (
              <a
                ...
                className={`block px-3 py-2 rounded-md text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-dark-hover transition-all duration-300 transform hover:translate-x-2 ${
                  activeSection === link.href.substring(1) ? 'bg-primary-light/10 text-primary-light' : ''
                }`}
